...
function ctct_settings_field( $args ) {
	...
	switch ( $args['type'] ) {
		case 'text':
			echo '<input name="ctct_settings[' . esc_attr( $args['id'] ) . ']" type="text" id="' . esc_attr( $args['id'] ) . '" value="' . esc_attr( $args['value'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" class="' . esc_attr( $args['class'] ) . '"/>';
			break;

		case 'checkbox':
			echo '<label for="' . esc_attr( $args['id'] ) . '"><input name="ctct_settings[' . esc_attr( $args['id'] ) . ']" type="checkbox" value="1" id="' . esc_attr( $args['id'] ) . '" ' . checked( 1, $args['value'], false ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" class="' . esc_attr( $args['class'] ) . '"/> ' . wp_kses_post( $args['description'] ) . '</label>';
			break;
	}
}
